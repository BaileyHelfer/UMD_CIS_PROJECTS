...
int
SplayTreeInventory::sizeHelper(Node* x)
{
	...
	if (x->left != NULL)
	{
		sumLeft += sizeHelper(x->left);
	
	}

	if (x->right != NULL)
	{
		sumRight += sizeHelper(x->right);
	
	}
	
	
	sum = sumLeft + sumRight + x->value;
}

int
SplayTreeInventory::size()
{
	Node* temp = stmap->root;
	int sum = sizeHelper(temp);
	return sum;
