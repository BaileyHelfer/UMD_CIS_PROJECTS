...
CompleteBT::getParentOfNewLastNode() {
    ...
    if (temp == NULL || temp->parent == root && temp == temp->parent->left) { 
        return temp->parent;
    }
    temp = firstLeftAncestor(temp);
    if (temp == NULL) { 
        temp = root;
    }
    else {
...
CompleteBT::getNewLastNode() {
    Node* z = this->lastNode->parent; // setting z to the parent of the last node
    if (z->parent == NULL) {
        return z;
    }
    if (z == NULL || z->parent == this->root && z == z->parent->left) {
        return z->parent;
    }
...
Heap::upHeapBubbling() {
    ...
    while (z != NULL && temp->elem->key->price < z->elem->key->price)
    {
        swap(temp, z);
        z = temp->parent;
    }
...
Heap::downHeapBubbling() {
    Node* temp = root;
    Node* z = minChild(temp);


    while (z != NULL && z->elem->key->price < temp->elem->key->price)
    {
        swap(z, temp);
        z = minChild(temp);
    }
