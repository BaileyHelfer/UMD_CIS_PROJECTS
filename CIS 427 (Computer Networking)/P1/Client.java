...
public class Client {
	...
	public static void main(String[] args) {
		...
		if (args.length < 1){
			...
			System.exit(1);
			}// Try to open a socket on SERVER_PORT// Try to open input and output streams
		try 
		{    
		...
		if (clientSocket != null && os != null && is != null) 
		{
			try     
			{
				while ((userInput = stdInput.readLine())!= null)
				{
					...
					//Switch statement to see if user wants to quit client side
					switch (serverInput)
					{
					//If user enters QUIT then close the connection to server and terminate 
					case "QUIT":
						os.close();
						is.close();
						clientSocket.close();
